...
function CreateForm({ type, onCreate, onDone }) {
  ...
  useEffect(() => { if (type === "products") api("/admin/categories?all=true").then(setCategories); }, [type]);
...
function AdminContent({ tab, data, request, remove }) {
  ...
  if (tab === "inventory") return <Table rows={data} columns={["product", "sku", "size", "color", "stock", "isActive"]} actions={(row) => <button onClick={() => { const stock = prompt("New stock", row.stock); if (stock !== null) patch(`/admin/variants/${row._id}/stock`, { stock: Number(stock) }); }}>Set stock</button>}/>;
...
function Table({ rows = [], columns, actions }) { return <div className="table-wrap"><table><thead><tr>{columns.map(column => <th key={column}>{column}</th>)}{actions && <th>Actions</th>}</tr></thead><tbody>{rows.map(row => <tr key={row._id}>{columns.map(column => <td key={column}>{display(row[column])}</td>)}{actions && <td className="table-actions">{actions(row)}</td>}</tr>)}</tbody></table>{!rows.length && <div className="empty-card">No records yet.</div>}</div>; }
